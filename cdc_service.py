...
from config import (
    ORACLE_CONFIG, AWS_CONFIG, SQS_QUEUE_URL,
...
class OracleCDCService:
    ...
    def connect_sqs(self) -> bool:
        ...
        sqs_logger.info(f"   Queue URL: {SQS_QUEUE_URL}")
    ...
    def validate_tables(self) -> bool:
        ...
        try:
            ...
            if exists:
                row_count = self._get_table_row_count(cursor, SOURCE_TABLE, SOURCE_SCHEMA)
                oracle_logger.info(f"   ✅ {SOURCE_SCHEMA}.{SOURCE_TABLE} exists ({row_count} rows)")
            else:
                oracle_logger.error(f"   ❌ {SOURCE_SCHEMA}.{SOURCE_TABLE} MISSING")
                all_valid = False
    ...
    def assemble_conversation(self, call_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            conversation = {
                ...
                'callTime': rows[0][5].isoformat() if rows[0][5] else None,  # CALL_TIME
                'messages': messages,
    ...
    # ============================
    # SQS Communication - Receive
    # ============================

    @log_function_call
    def receive_ml_results(self):
        """Poll SQS for ML processing results"""
        sqs_logger.debug("📥 Polling SQS for ML results...")
        ...
        try:
            ...
            if messages:
                sqs_logger.info(f"📩 Received {len(messages)} message(s) from SQS")
    ...
    def write_ml_result(self, result: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if isinstance(sentiment_data, dict):
                ...
                sentiment = str(sentiment_data) if sentiment_data else 'unknown'

            # Handle classification - can be dict or string
            classification_data = result.get('classification', {})
            if isinstance(classification_data, dict):
                classification = classification_data.get('primary', 'unknown')
            ...
            oracle_logger.debug(f"   Sentiment: {sentiment}")
            oracle_logger.debug(f"   Classification: {classification}")
            oracle_logger.debug(f"   Confidence: {result.get('confidence', 0)}")
            ...
            """, {
                'call_id': call_id,
                'customer_id': result.get('ban'),
                'subscriber_no': result.get('subscriberNo'),
                'call_time': result.get('callTime'),
                'summary': summary_text,
                'sentiment': sentiment,
                'classification': classification,
                'all_classifications': json.dumps(all_classifications),
                'confidence': result.get('confidence'),
                'processing_time': result.get('processingTime'),
                'model_version': result.get('modelVersion', 'dictalm-2.0')
            })
